...
class UI_QuanLySinhVien(object):
        ...
        def xoa_file_anh_sinh_vien(self, masinhvien):
                import os
                import glob
                
                BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                
                # Quét và xóa các ảnh dataset trong thư mục image/photo
                photo_dir = os.path.join(BASE_DIR, 'image', 'photo')
                photo_pattern = os.path.join(photo_dir, f"*{masinhvien}*.jpg") 
                for file_path in glob.glob(photo_pattern):
                        try:
                                os.remove(file_path)
                        except:
                                pass

                # Quét và xóa các ảnh lúc điểm danh
                diemdanh_dir = os.path.join(BASE_DIR, 'image', 'diemdanh')
                diemdanh_pattern = os.path.join(diemdanh_dir, f"*{masinhvien}*.jpg")
                for file_path in glob.glob(diemdanh_pattern):
                        try:
                                os.remove(file_path)
                        except:
                                pass
        ...
        def delete(self):
                svBUS = SinhVienBUS()
                masinhvien_can_xoa = self.ma
                if(svBUS.delete(self.ma)):
                        QMessageBox.information(self.centralwidget,"Thông báo","Xóa thành công")
                        # GỌI HÀM DỌN RÁC SAU KHI XÓA THÀNH CÔNG
                        self.xoa_file_anh_sinh_vien(masinhvien_can_xoa)
                        self.loadDataQTable()
                        self.clear()
                else:
        ...
        def trainImage(self):
                QMessageBox.information(self.centralwidget, "Thông báo", "Hệ thống đang chạy ngầm Train Model. Vui lòng đợi thông báo hoàn tất...")
                self.thread_train = face_training()
                self.thread_train.finished.connect(self.train_finished)
                self.thread_train.start()

        def train_finished(self, success, message):
                if success:
                        QMessageBox.information(self.centralwidget, "Hoàn tất", message)
                else:
                        QMessageBox.warning(self.centralwidget, "Lỗi", message)
